"""https://github.com/ptirupat/MLAD/blob/main/src/cooccur_metric.py"""
import numpy as np
...
def conditional_t(y_pred, y_gt, y_gt_mask, thresh=0.5, avg=True):
    ...
    for c_j in range(n_classes):
        ...
        # if this is the case, then there are no actions happening before/after c_j
        # the fix is to directly set prec/rec/n_occurs/maps to 0, or pass
        if y_gt_sub.shape[0] > 0:
...
def get_all_conditional_metrics(y_pred, y_gt, t=0, thresh=0.5, avg=True):
    prec, re, ns, map = conditional_metric(y_pred, y_gt, t=t, thresh=thresh, avg=avg)
    fs = get_f1(prec, re) # action conditional f1-score
    prec, re, map, fs = np.nan_to_num([prec, re, map, fs])

    return prec*100, re*100, map, fs*100, ns
